...
void microphoneTask(void* param) {
  ...
  while (true) {
		...
		if ((event.flag != EMIC_START && event.state != AUDIO_STATE_RUNNING)
			...
			|| event.collectorCallback == nullptr
			|| event.executorCallback == nullptr) {
      goto end;
    }
    ...
    // Publish start
		...
		if (event.collectorCallback)
			event.collectorCallback(key, index, chunk, maxSamples);
		index++;
