...
bool WeatherService::init(const WeatherConfig& config) {
    ...
    Logger::info("Weather", "Weather service initialized");
    return true;
}
...
void WeatherService::fetchFromAPI(WeatherCallback callback) {
    ...
    Logger::info("Weather", "Fetching weather from: %s", url.c_str());
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        Logger::error("Weather", "HTTP error: %d", httpCode);
        if (callback) {
            WeatherData errorData;
            callback(errorData, false);
        }
...
void WeatherService::processAPIResponse(const String& response, WeatherCallback callback) {
    if (!callback) {
        Logger::warn("Weather", "No callback provided");
        return;
    }

    Logger::info("Weather", "Processing API response");

    JsonDocument doc;
    DeserializationError error = deserializeJson(doc, response);

    if (error) {
        Logger::error("Weather", "JSON parsing failed: %s", error.c_str());
        WeatherData errorData;
        callback(errorData, false);
        return;
    }
    ...
    try {
        if (doc["lokasi"].isUnbound()) {
            Logger::error("Weather", "No lokasi found in response");
            callback(data, false);
            return;
        }
        ...
        Logger::info("Weather", "Location: %s (Lat: %.6f, Lon: %.6f)", data.location.c_str(), data.latitude, data.longitude);

        if (doc["data"].isUnbound() || doc["data"].size() == 0) {
            Logger::error("Weather", "No data array found");
            callback(data, false);
            return;
        }
        ...
        if (dataArray.size() == 0) {
            Logger::error("Weather", "Empty data array");
            callback(data, false);
            return;
        }
        ...
        if (locationData["cuaca"].isUnbound() || locationData["cuaca"].size() == 0) {
            Logger::error("Weather", "No cuaca data found");
            callback(data, false);
            return;
        }
        ...
        if (cuacaArray.size() == 0) {
            Logger::error("Weather", "No current weather data");
            callback(data, false);
            return;
        }
        ...
        if (timePeriod.size() == 0) {
            Logger::error("Weather", "No weather data in time period");
            callback(data, false);
            return;
        }
        ...
        Logger::info("Weather", "Weather: %s (Code: %d)", data.description.c_str(), weatherCode);
        Logger::info("Weather", "Temp: %d°C, Humidity: %d%%, Wind: %d km/h %s",
        ...
        Logger::error("Weather", "Exception during processing");
        WeatherData errorData;
        callback(errorData, false);
    }
}
...
bool WeatherService::saveCache(const WeatherData& data) {
    ...
    if (!file) {
        Logger::error("Weather", "Failed to open cache file for writing");
        return false;
    }
    ...
    Logger::debug("Weather", "Weather data cached");
    return true;
}
