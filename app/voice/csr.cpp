...
esp_err_t sr_start(bool feedCore, bool detectCore) {
  //Start tasks
  esp_err_t ret_val = xTaskCreatePinnedToCoreWithCaps(&SR::audio_feed_task, "SR Feed Task", 4 * 1024, NULL, 6, &SR::g_sr_data->feed_task, feedCore, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if(pdPASS != ret_val) {
  ...
  vTaskDelay(10);
  ret_val = xTaskCreatePinnedToCoreWithCaps(&SR::audio_detect_task, "SR Detect Task", 8 * 1024, NULL, 6, &SR::g_sr_data->detect_task, detectCore, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if(pdPASS != ret_val) {
  ...
  ret_val = xTaskCreatePinnedToCoreWithCaps(&SR::sr_handler_task, "SR Handler Task", 6 * 1024, NULL, configMAX_PRIORITIES - 1, &SR::g_sr_data->handle_task, feedCore, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if(pdPASS != ret_val) {
